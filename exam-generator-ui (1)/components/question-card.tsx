...
export function QuestionCard({ question, index }: QuestionCardProps) {
  ...
  const isCorrect = selectedAnswer && selectedAnswer.startsWith(question.correctAnswer)
    ...
    <Card className="overflow-hidden animate-fade-in-up shadow-sm hover:shadow-md transition-shadow">
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <div className="prose prose-sm max-w-none">
          <p className="text-foreground leading-relaxed font-medium">{question.question}</p>
        </div>
        ...
        <div className="space-y-2.5">
          {question.options.map((option: string, idx: number) => {
            ...
            const isCorrectOption = option.startsWith(question.correctAnswer)
        ...
        <div className="pt-4 border-t">
          ...
          {showAnswer && (
            <div className="mt-4 p-4 bg-primary/5 border border-primary/20 rounded-xl animate-fade-in">
              <p className="text-sm font-semibold text-primary mb-2">Doğru Cevap: {question.correctAnswer}</p>
