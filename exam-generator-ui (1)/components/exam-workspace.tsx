...
export function ExamWorkspace({ config, onBack }: ExamWorkspaceProps) {
  ...
  const loadQuestionsFromAPI = async () => {
    try {
      ...
      while (true) {
        ...
        // Son satırı buffer'da tut (henüz tam olmayabilir)
        buffer = lines.pop() || ""
        ...
        for (const line of lines) {
          if (line.trim()) {
            try {
              ...
              console.log("[v0] Soru alındı:", question.id, question.hasImage ? "(görselli)" : "")
              setQuestions((prev) => {
              ...
              console.error("[v0] JSON parse hatası:", e)
            }
          }
        }
      }
  ...
  const allQuestionsLoaded = questions.length === totalQuestions
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="text-sm font-medium border-2 px-3 py-1.5">
              {questions.length} / {totalQuestions} Soru Yüklendi
            </Badge>
            {allQuestionsLoaded && (
              <Button
